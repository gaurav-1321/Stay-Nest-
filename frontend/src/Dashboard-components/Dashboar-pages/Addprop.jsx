...
const Addprop = () => {
  ...
  const handlesubmit = async (e) => {
    ...
    // 1. Retrieve the stored token from authentication/login phase
    const token = localStorage.getItem("token"); 

    try { 
      const res = await fetch("http://localhost:5000/api/prop/create", {
        method: "POST",
        headers: {
      ...
      if (res.ok) {
        alert("Property added to Database");
        setdesc("");
        setimages("");
        setlocation("");
        setprice("");
  ...
  return (
    <div>
      <h2 className="text-4xl font-serif font-bold text-center text-gray-800 mb-8">
        Add Your Property & <span>Be part of our us.</span>
      </h2>
      <form className="space-y-6" onSubmit={handlesubmit}>
        ...
        <div className="flex flex-col space-y-4">
          <label className="text-lg font-serif font-semibold mb-2">Images</label>
        ...
        <div className="flex flex-col space-y-4">
          ...
          />
          <button 
            className="bg-blue-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-blue-700 shadow-md" 
            type="submit"
          >
            Add Your Property
          </button>
